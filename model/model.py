...
class SpatialSELayer(nn.Module):
    ...
    def forward(self, input_tensor, main_feature, weights=None):
        ...
        # spatial excitation
        output_tensor = torch.mul(main_feature, squeeze_tensor.view(batch_size, 1, a, b))
...
class ChannelSELayer(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        # fc_out_1 = F.dropout(fc_out_1, p=0.5, training=self.training)
        # fc_out_1 = self.relu(fc_out_1)
        # fc_out_2 = self.fc2(fc_out_1)
        # # fc_out_2 = F.dropout(fc_out_2, p=0.5, training=self.training)
        # fc_out_2 = self.sigmoid(fc_out_2)
...
class BoundaryMapping5(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.conv3x3(x)
        # b_4 = self.conv(x)
        # so_output_feature_1 = self.conv1(b_4)
        # so_output_feature = self.upsample2(so_output_feature_1)
        # so_output_feature = self.conv2(so_output_feature)
        # so_output_feature = self.conv3(so_output_feature)
        # so_output = self.bn1(so_output_feature)
        # so_output = self.upsample2(so_output)
        # so_output = self.conv4(so_output)
        # so_output = self.upsample2(so_output)
...
class BoundaryMapping2(nn.Module):
    ...
    def forward(self, x_1):
        # x_1 = self.conv(x_1)
        # b_2 = self.upsample2(b_2)
        # snr_2 = self.upsample2(snr_2)
        ...
        so_output = self.conv3(so_output)
        # so_output = self.upsample2(so_output)
        # map = self.downsample4(so_output)
...
class SNR(nn.Module):
    ...
    def __init__(self, input_channels, reduction_ratio=16):
        ...
        self.reduction_ratio = reduction_ratio
        # self.InstanceNorm = SwitchableNorm2D(self.input_channels)
        ...
        # self.SP = SpatialSELayer(self.input_channels)
...
class SNR1(nn.Module):
    def __init__(self, input_channels, reduction_ratio=16):
        super(SNR1, self).__init__()
        self.input_channels = input_channels
        self.reduction_ratio = reduction_ratio
        # self.InstanceNorm = SwitchableNorm2D(self.input_channels)
        self.InstanceNorm = nn.InstanceNorm2d(self.input_channels, affine=True)
        self.SE = ChannelSELayer(self.input_channels, self.reduction_ratio)
        # self.SP = SpatialSELayer(self.input_channels)
        self.conv = nn.Sequential(nn.Conv2d(input_channels, input_channels, 1),
                                  nn.BatchNorm2d(input_channels),
                                  nn.ReLU())
        self.conv2 = nn.Sequential(nn.Conv2d(input_channels * 2, input_channels, 1),
                                   nn.BatchNorm2d(input_channels),
                                   nn.ReLU())
        self.upsample2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)

    # f is feature layer, b is boundary feature, snr_pr is snr previous
    def forward(self, f, b, snr_pr):
        f = self.conv(f)
        b = self.conv(b)
        b = b + f
        b = self.conv(b)
        snr_pr = self.upsample2(snr_pr)
        snr_pr = self.conv2(snr_pr)
        snr_pr = snr_pr * f
        snr_pr = self.conv(snr_pr)
        seg_feature = b + snr_pr + f
        seg_feature = self.conv(seg_feature)
        return seg_feature


class SNR2(nn.Module):
    def __init__(self, input_channels, reduction_ratio=16):
        super(SNR2, self).__init__()
        self.input_channels = input_channels
        self.reduction_ratio = reduction_ratio
        # self.InstanceNorm = SwitchableNorm2D(self.input_channels)
        self.InstanceNorm = nn.InstanceNorm2d(self.input_channels, affine=True)
...
class SNR4(nn.Module):
    def __init__(self, input_channels, reduction_ratio=16):
        super(SNR4, self).__init__()
        self.input_channels = input_channels
        self.reduction_ratio = reduction_ratio
        # self.InstanceNorm = SwitchableNorm2D(self.input_channels)
        self.InstanceNorm = nn.InstanceNorm2d(self.input_channels, affine=True)
        ...
        self.conv = nn.Sequential(nn.Conv2d(input_channels, input_channels, 1),
                                  nn.BatchNorm2d(input_channels),
                                  nn.ReLU())
        self.conv2 = nn.Sequential(nn.Conv2d(input_channels, input_channels // 2, 1),
...
class GenMap(nn.Module):
    def __init__(self, num_input, kernel_sz=None, stride=None, padding=None):
...
class SharedEncoder(nn.Module):
    def __init__(self):
        ...
        # self.layer_0 = nn.Sequential(Seg_Model.conv1, Seg_Model.bn1, Seg_Model.relu)
        ...
        # self.fusemodule_2 = FuseModule(640, 512)
        # self.fusemodule_3 = FuseModule(1280, 1024)
        self.score_final = nn.Conv2d(4, 1, 1)  # map
        self.smooth_p = nn.Linear(128, 1)
        # self.peeler = HIPePeeler()
        # self.guider = Guider()
        ...
        # self.sp_1 = SpatialSELayer(64)
        # self.sp_2 = SpatialSELayer(256)
        # self.sp_3 = SpatialSELayer(512)
        # self.sp_4 = SpatialSELayer(1024)
    ...
    def forward(self, x, aux=None, original=None):
        ...
        if aux is not None:
            ...
            ratio = torch.randn(1, device='cuda') + smooth_param_2
        ...
        if aux is not None:
            x_rest = x_rest * 2 - 1
            # x_rest = (x_rest + 1) / 2
            return x_x, x_rest, x3, x4, results, boundary_1, boundary_2, boundary_3, boundary_4
        else:
            return x_x, x3, x4
